...
global is_image_finished
if_image_finished = False
...
def save_automatic_photo():
    global is_possible_to_save, starting_number, updated_starting_number, speed_turtle, color_pen, numero_diapositiva, numero_passi, is_possible_to_save, numero_lati, color_prime, canvas_color
    ...
    for i in range(iterations_number):
        ...
        numero_passi += 1
    t.hideturtle()
    # We need to update the screen if the speed is 0, because tracer(0) for no animation need screen.update()
    # If we not update the screen the foto will be done before the image is generated because screen.update() is also
    ...
    # When spedd is set to 0 (maximum speed) we use tracer to create the image in one step
    # At the end there is the command screen.update()
    # Here we write is_possible_to_save = True
...
screen.update()
# this line is for speed=0 (maximum speed). Tracer(0) let the image to being created in one passage without animations
is_possible_to_save = True
root.mainloop()
